...
const _extract = require("../dist/extract-abbreviation.es").default;
...
describe("Extract abbreviation", () => {
	...
	it("basic", () => {
		...
		assert.deepEqual(
			extract('foo div[foo="текст" bar=текст2]'),
	...
	it("abbreviation with operators", () => {
		...
		assert.deepEqual(
			extract("a foo>bar+baz*3"),
			result("foo>bar+baz*3", 2)
		);
	});
	...
	it("abbreviation with attributes", () => {
		...
		assert.deepEqual(
			extract('a foo[bar="baz" a b]'),
			result('foo[bar="baz" a b]', 2)
		);
	...
	it("tag test", () => {
		assert.deepEqual(
			extract('<foo>bar[a b="c"]>baz'),
			result('bar[a b="c"]>baz', 5)
		);
